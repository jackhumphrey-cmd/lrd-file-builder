...
st.markdown("Generate recurring schedule migration files with optional mapping file support.")
...
if token_file and schedule_file:
    ...
    with st.spinner("Processing data..."):
        ...
        if mapping_file:
            ...
            # Merge tokens directly into schedule (original behavior)
            merged = schedule.merge(
                tokens[["source_old_id", "created_customer", "source_new_id"]],
    ...
    if len(problem_rows) > 0:
        ...
        st.download_button("Download Problem Rows", problem_csv, "migration_problem_rows.csv", "text/csv")        tokens = tokens.drop(columns=[c for c in ["old_id"] if c in tokens.columns])
        ...
        if mapping_file:
            ...
            # Rename mapping columns for consistency
            mapping_df = mapping_df.rename(columns={
                "reference_token": "source_old_id",
                "stax_payment_method_id": "Gateway_PaymentTokenId"
            })
            ...
            # Merge tokens directly into schedule
            merged = schedule.merge(
                tokens[["source_old_id", "created_customer", "source_new_id"]],
                left_on="Gateway_PaymentTokenId",
                right_on="source_old_id",
